...
### 3. 合并已有的 SVF `wpa --print-fp` 结果

```bash
build/ir_graph_extractor ../input.bc -o ../output \
  --svf-wpa-output ../output/svf_wpa.out
```

### 4. 一键运行 SVF 并合并

```bash
build/ir_graph_extractor ../input.bc -o ../output --run-svf
```

如果 `wpa` 不在默认位置，可以显式指定：

```bash
build/ir_graph_extractor ../input.bc -o ../output \
  --run-svf --svf-bin /path/to/wpa
```

也可以使用独立脚本：

```bash
./scripts/extract_with_svf.sh ../input.bc ../output
```
...
- `callsites[*]` 是调用点级信息，保留 direct/indirect 区分

`callsites[*]` 关键字段：

- `kind`: `direct` 或 `indirect`
- `function`: 所属函数 ID
- `basic_block`: 所属基本块 ID
- `line`: 调用点行号
- `callee`: direct call 的目标函数 ID
- `resolved_targets`: indirect call 的候选目标 ID
- `source_kind`: 间接调用值来源，如 `arg/load/global/phi/select/ret_of_call`
- `source_callee`: 若 `source_kind=ret_of_call`，记录来源 call 的 callee
- `ir`: 调用点的 LLVM IR 文本
...
- `functions[*].refs` 会额外并入 `SVF` 解出的间接调用候选
- `output/svf_wpa.out` 和 `output/svf_wpa.err` 会保存在输出目录
...
2. `SVF` 回填当前通过 `basename + line + call 指令文本` 匹配调用点，大项目上会有未命中
3. CFG 只到函数内，不包含调用边和返回边
4. 外部库调用没有 summary 建模
5. 没有上下文敏感分析
