...
struct CallSiteInfo {
  ...
  uint32_t SourceCalleeId = 0;
  bool HasSourceCallee = false;
  std::vector<uint32_t> ResolvedTargets;
};
...
static cl::opt<std::string> SVFWpaOutput(
    "svf-wpa-output",
    cl::desc("Optional SVF wpa --print-fp output to merge indirect call targets"),
    cl::init(""));

static cl::opt<bool> RunSVF(
    "run-svf",
    cl::desc("Run SVF wpa --ander --print-fp and merge indirect call targets"),
    cl::init(false));

static cl::opt<std::string> SVFBin(
    "svf-bin",
    cl::desc("Path to SVF wpa binary"),
    cl::init("/home/lab420/Desktop/svf/third_party/SVF/Release-build/bin/wpa"));
...
static std::string getBaseName(StringRef Path) {
  SmallString<256> Buffer(Path);
  return sys::path::filename(Buffer).str();
}

static std::string normalizeWhitespace(StringRef Text) {
  std::string Out;
...
static std::string instructionToKey(const Instruction &I) {
  std::string Text;
...
public:
  ...
  const std::vector<CallSiteInfo> &getCallSites() const { return CallSites; }

  bool mergeSVFWpaOutput(StringRef Path, raw_ostream &Log) {
    if (Path.empty()) {
      return true;
    }

    auto SitesOrErr = parseSVFWpaOutput(Path, Log);
    if (!SitesOrErr) {
      return false;
    }

    std::unordered_map<std::string, std::vector<const IndirectCallSite *>> SiteMap;
    for (const auto &Site : IndirectCalls) {
      std::string Key = buildIndirectSiteKey(Site);
      if (!Key.empty()) {
        SiteMap[Key].push_back(&Site);
      }
    }

    size_t MatchedSites = 0;
    size_t AddedRefs = 0;
    size_t UnmatchedSites = 0;
    for (const auto &SVFSite : *SitesOrErr) {
      std::string Key = buildSVFSiteKey(SVFSite);
      if (Key.empty()) {
        continue;
      }
      ...
      for (const std::string &TargetName : SVFSite.Targets) {
        if (const Function *Target = findFunctionByName(TargetName)) {
          Targets.insert(Target);
        }
      }

      for (const IndirectCallSite *Site : It->second) {
        size_t Before = Functions[Site->FunctionId].Refs.size();
        recordResolvedTargets(Site->FunctionId, Site->BlockId, Targets,
                              Site->CallSiteId);
        AddedRefs += Functions[Site->FunctionId].Refs.size() - Before;
      }
    }
    ...
    Log << "svf merge finished\n";
    Log << "  svf_sites: " << SitesOrErr->size() << "\n";
    Log << "  matched_sites: " << MatchedSites << "\n";
    Log << "  unmatched_sites: " << UnmatchedSites << "\n";
    Log << "  refs_added: " << AddedRefs << "\n";
    return true;
  }
...
private:
  ...
  struct ValueSourceInfo {
    std::string Kind = "unknown";
    const Function *SourceCallee = nullptr;
  };

  static ValueSourceInfo classifyValueSource(const Value *V) {
    ValueSourceInfo Info;
    if (!V) {
      return Info;
    }
    ...
    if (isa<Argument>(V)) {
      ...
      Info.Kind = "ret_of_call";
      Info.SourceCallee = CB->getCalledFunction();
  ...
  uint32_t recordCallSite(const CallBase &CB, uint32_t FunctionId, uint32_t BlockId,
                          StringRef Kind, const Function *DirectCallee = nullptr) {
    uint32_t Id = static_cast<uint32_t>(CallSites.size());
    CallSiteInfo Info;
    Info.Id = Id;
    Info.FunctionId = FunctionId;
    Info.BlockId = BlockId;
    Info.Kind = Kind.str();
    Info.IR = instructionToKey(CB);

    std::string FileName;
    uint32_t Line = 0;
    getDebugLoc(CB, FileName, Line);
    Info.FileName = FileName;
    Info.Line = Line;

    std::string Expr;
    raw_string_ostream ExprOS(Expr);
    CB.getCalledOperand()->printAsOperand(ExprOS, false);
    ExprOS.flush();
    Info.CalleeExpr = normalizeWhitespace(Expr);

    if (DirectCallee && !shouldSkipFunction(*DirectCallee)) {
      Info.CalleeId = recordFunction(*DirectCallee);
      Info.HasCallee = true;
    } else {
      ValueSourceInfo Source = classifyValueSource(CB.getCalledOperand());
      Info.SourceKind = Source.Kind;
      if (Source.SourceCallee && !shouldSkipFunction(*Source.SourceCallee)) {
        Info.SourceCalleeId = recordFunction(*Source.SourceCallee);
        Info.HasSourceCallee = true;
      }
    }
  ...
  const Function *findFunctionByName(StringRef Name) const {
    for (const Function &F : M) {
      ...
      if (demangle(F.getName().str().c_str()) == Name) {
        return &F;
      }
    }
    return nullptr;
  ...
  std::string buildIndirectSiteKey(const IndirectCallSite &Site) const {
    if (!Site.CB) {
      return "";
    }
    std::string FileName;
    uint32_t Line = 0;
    getDebugLoc(*Site.CB, FileName, Line);
    if (Line == 0) {
      return "";
    }
    return (Twine(getBaseName(FileName)) + ":" + Twine(Line) + ":" +
            instructionToKey(*Site.CB))
        .str();
  }

  static std::string buildSVFSiteKey(const SVFWpaSite &Site) {
    if (Site.Line == 0 || Site.InstructionText.empty()) {
      return "";
    }
    return (Twine(getBaseName(Site.FileName)) + ":" + Twine(Site.Line) + ":" +
            normalizeWhitespace(Site.InstructionText))
        .str();
  }

  static Expected<std::vector<SVFWpaSite>>
  parseSVFWpaOutput(StringRef Path, raw_ostream &Log) {
    auto BufferOrErr = MemoryBuffer::getFile(Path);
    if (!BufferOrErr) {
      return createStringError(inconvertibleErrorCode(),
                               "failed to read SVF WPA output: " +
                                   BufferOrErr.getError().message());
    }

    Regex CallSiteRe(
        "^CallSite:[[:space:]]+(.*)[[:space:]]+Location: \\{ ln: ([0-9]+) fl: ([^}]*) \\}[[:space:]]+with Targets:[[:space:]]*$");
    std::vector<SVFWpaSite> Sites;
    SmallVector<StringRef, 0> Matches;
    SVFWpaSite Current;
    bool InSection = false;
    bool InSite = false;
    SmallVector<StringRef, 0> Lines;
    StringRef(BufferOrErr.get()->getBuffer()).split(Lines, '\n');
    for (StringRef Line : Lines) {
      StringRef Trimmed = Line.trim();
      if (!InSection) {
        if (Trimmed == "==================Function Pointer Targets==================") {
          InSection = true;
        }
        continue;
      }
      ...
      if (Trimmed.empty()) {
        if (InSite) {
          Sites.push_back(std::move(Current));
          Current = SVFWpaSite();
          InSite = false;
        }
        continue;
      ...
      Matches.clear();
      if (CallSiteRe.match(Trimmed, &Matches)) {
        if (InSite) {
          Sites.push_back(std::move(Current));
          Current = SVFWpaSite();
        }
        Current.InstructionText = Matches[1].str();
        Matches[2].getAsInteger(10, Current.Line);
        Current.FileName = Matches[3].str();
        InSite = true;
        continue;
      }

      if (InSite) {
        Current.Targets.push_back(Trimmed.str());
      }
    }
    ...
    Log << "parsed svf sites: " << Sites.size() << "\n";
    return Sites;
  }
  ...
  void recordResolvedTargets(uint32_t FunctionId, uint32_t BlockId,
    ...
    for (const Function *Target : Targets) {
      ...
      if (CallSiteId != UINT32_MAX && CallSiteId < CallSites.size()) {
        CallSites[CallSiteId].ResolvedTargets.push_back(TargetId);
      }
    }
  }
      ...
      for (const BasicBlock &BB : F) {
        ...
        for (const Instruction &I : BB) {
          ...
          if (const Function *Callee = CB->getCalledFunction()) {
            ...
            recordCallSite(*CB, FuncId, BlockId, "direct", Callee);
          ...
          uint32_t CallSiteId = recordCallSite(*CB, FuncId, BlockId, "indirect");
          IndirectCalls.push_back({&F, &BB, CB, FuncId, BlockId, CallSiteId});
        }
      }
    }
  }
  ...
  void finalizeFunctions() {
    ...
    for (CallSiteInfo &Info : CallSites) {
      dedup(Info.ResolvedTargets);
    }
...
static void writeStaticJson(StringRef Path, const std::vector<FunctionInfo> &Functions,
  ...
  json::Array CallSiteEntries;
  for (const auto &CS : CallSites) {
    json::Object Entry;
    Entry["id"] = CS.Id;
    Entry["function"] = CS.FunctionId;
    Entry["basic_block"] = CS.BlockId;
    Entry["line"] = CS.Line;
    Entry["kind"] = CS.Kind;
    Entry["ir"] = CS.IR;
    Entry["callee_expr"] = CS.CalleeExpr;
    if (!CS.FileName.empty()) {
      Entry["file_name"] = CS.FileName;
    }
    if (CS.HasCallee) {
      Entry["callee"] = CS.CalleeId;
    }
    if (!CS.SourceKind.empty()) {
      Entry["source_kind"] = CS.SourceKind;
    }
    if (CS.HasSourceCallee) {
      Entry["source_callee"] = CS.SourceCalleeId;
    }
    Entry["resolved_targets"] = toJsonArray(CS.ResolvedTargets);
    CallSiteEntries.push_back(std::move(Entry));
  }

  json::Object Root;
  Root["basic_blocks"] = std::move(BlockEntries);
  Root["functions"] = std::move(FunctionEntries);
  Root["callsites"] = std::move(CallSiteEntries);
...
static bool runSVFWpa(StringRef Input, StringRef BaseDir, raw_ostream &Log,
                      std::string &WpaOutputPath) {
  SmallString<256> OutputPath(BaseDir);
  sys::path::append(OutputPath, "svf_wpa.out");
  SmallString<256> ErrorPath(BaseDir);
  sys::path::append(ErrorPath, "svf_wpa.err");
  WpaOutputPath = OutputPath.str().str();

  std::string Redirects[3];
  Redirects[1] = WpaOutputPath;
  Redirects[2] = ErrorPath.str().str();

  SmallVector<StringRef, 4> Args;
  Args.push_back(SVFBin);
  Args.push_back("--ander");
  Args.push_back("--print-fp");
  Args.push_back(Input);

  Optional<StringRef> RedirectRefs[3];
  RedirectRefs[1] = Redirects[1];
  RedirectRefs[2] = Redirects[2];
  ...
  std::string ErrMsg;
  int RC = sys::ExecuteAndWait(SVFBin, Args, None, RedirectRefs, 0, 0, &ErrMsg);
  if (RC != 0) {
    errs() << "failed to run SVF wpa: " << ErrMsg << "\n";
    errs() << "  see: " << ErrorPath << "\n";
    return false;
  }
  return true;
...
int main(int argc, const char **argv) {
  ...
  Analyzer Analyzer(*M);
  Analyzer.run();
  std::string EffectiveSVFWpaOutput = SVFWpaOutput;
  if (RunSVF) {
    if (!runSVFWpa(InputPath, OutputDir, outs(), EffectiveSVFWpaOutput)) {
      return 1;
    }
  }
  if (!EffectiveSVFWpaOutput.empty()) {
    if (!Analyzer.mergeSVFWpaOutput(EffectiveSVFWpaOutput, outs())) {
      return 1;
    }
  }
